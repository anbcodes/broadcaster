...
export const actions ={
    default: async ({request, fetch}) => {
        ...
        if (typeof content !== 'string' || typeof include !== 'string' || typeof exclude !== 'string') {
            return fail(400, {content: '', include: '', exclude: '', error: 'Invalid input'});
        }
        ...
        const result = await (await fetch('./posts/add.json', {
            ...
            body: JSON.stringify({content, include: include.split(',').filter(v => v.trim()), exclude: exclude.split(',').filter(v => v.trim())})
        })).json();
