<script>
    ...
    const usersReferenced = posts.map(post => {
    ...
    const viewableList = usersReferenced.map(user => {
        return `u:${user}`;
    }).concat("error" in data.groups ? [] : data.groups.map(v=>`g:${v.name}`));
...
<div class="prose w-full max-w-full pt-10">
    ...
    <form bind:this={formEl} class="form-control" method="POST">
        <PostEditor content={form?.content} viewableAutocomplete={viewableList} viewableTo={form?.viewableTo} user={data.user}></PostEditor>
    ...
    {#each sortedPosts as post}
        <div class="mt-4 text-sm">{getRelativeTime(Temporal.ZonedDateTime.from(post.created))}
